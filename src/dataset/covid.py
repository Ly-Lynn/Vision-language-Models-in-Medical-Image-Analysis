...
class COVIDDataset(BaseClassificationDataset):
    ...
    def _load_data(self) -> pd.DataFrame:
        ...
        for data in self.datalist:
            filename = os.path.join(self.data_root, f'{data}-meta.csv')
    ...
    def _prepare_data(self):
        ...
        def create_covid_csv(covid_lst, non_covid_lst, split):
            ...
            for non_covid in non_covid_lst:
                ...
                df = pd.concat([df, new_row], ignore_index=True)
            df.to_csv(f'local_data/covid-{split}-meta.csv', index=True)
            logger.info(f'Created COVID data')
            return df
        try:
...
class COVIDZeroShotCollator(BaseCollator):
    ...
    def _process_class_prompts(self) -> Dict[str, Any]:
        ...
        try:
            from utils import generate_covid_class_prompts
        except ImportError:
